...
const { deployERC721Token } = require('../scripts/deployERC721Token.js');
...
describe('MessageQueueTest', async function () {
  ...
  before(async function () {
    erc721TokenAddress = await deployERC721Token();
    erc721Token = await ethers.getContractAt('ERC721Token', erc721TokenAddress);
    erc721TokenWallet = erc721Token.connect(wallet);
  ...
  it('test erc721token transferOwnership', async () => {
    const tx = await erc721Token.transferOwnership(wallet.address);
    await tx.wait();
    assert.equal(await erc721Token.owner(), wallet.address);
  });
  ...
  it ('test mint721token succeed', async () => {
    const tokenId = 1; 
    const tx = await erc721TokenWallet.mint(wallet.address, tokenId);
    await tx.wait();
    const receipt = await ethers.provider.getTransactionReceipt(tx.hash);

    const erc721Balance = await erc721Token.balanceOf(wallet.address);
    const name = await erc721Token.name();

    assert.equal(erc721Balance, 1);
  });

  it ('test depositERC721Token succeed', async () => {
    const testAmount = ethers.parseEther('1');
    ...
    const approveTx = await erc721TokenWallet.approve(tokenTransmitterWallet.target, 1);
    await approveTx.wait();
    const approveTxReceipt = await ethers.provider.getTransactionReceipt(approveTx.hash);

    const tx = await tokenTransmitterWallet.depositERC721Token(
      erc721TokenWallet.target,
      recipient,
      1,
      0,
      { value: testAmount }
  ...
  it('test depositERC721Token p -> c', async () => {
    const testAmount = ethers.parseEther('1');
    const testErc20Amount = '20000';
  ...
  it ('test depositERC721Token balance', async () => {
    const tokenId = 1; 
    const tx721 = await erc721TokenWallet.mint(wallet.address, tokenId);
    await tx721.wait();
    const receipt721 = await ethers.provider.getTransactionReceipt(tx721.hash);
