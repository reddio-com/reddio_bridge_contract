...
async function deployChildLayerDiamond(deployedContractsMap = {}) {
  ...
  let libChildLayerTokenStorageAddress;
  const FacetNames = [
    'DiamondLoupeFacet',
    'OwnershipFacet',
    'ERC20TokenFactoryFacet',
    'ERC721TokenFactoryFacet',
    'ERC1155TokenFactoryFacet',
    'DownwardMessageDispatcherFacet',
    'LibChildLayerTokenStorage',
    'ChildBridgeCoreFacet',
    'ChildTokenMessageProcessorFacet',
    'ChildTokenMessageTransmitterFacet',
  ];
  const cut = [];
  for (const FacetName of FacetNames) {
    console.log('libChildLayerTokenStorageAddress', libChildLayerTokenStorageAddress);
    let libraries = {};
    if (!!libChildLayerTokenStorageAddress) {
      libraries = {
        LibChildLayerTokenStorage: libChildLayerTokenStorageAddress,
      };
    }
    ...
    if (FacetName == 'LibChildLayerTokenStorage') {
      console.log('here');
      libChildLayerTokenStorageAddress = facet.target;
    }
    cut.push({
...
exports.deployChildLayerDiamond = deployChildLayerDiamond;
