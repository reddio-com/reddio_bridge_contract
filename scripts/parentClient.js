...
async function main() {
  ...
  const parentStateVerifierFacetAbi = await readArtifactAbi('ParentStateVerifierFacet');
  const upwardMessageDispatcherFacetAbi = await readArtifactAbi('UpwardMessageDispatcherFacet');
  const parentTokenMessageProcessorFacetAbi = await readArtifactAbi('ParentTokenMessageProcessorFacet');
  const parentTokenMessageTransmitterFacetAbi = await readArtifactAbi('ParentTokenMessageTransmitterFacet');
  const accessControlFacetAbi = await readArtifactAbi('AccessControlFacet');
  const TimelockFacetAbi = await readArtifactAbi('TimelockFacet');
  ...
  const facetChoices = [
    ...
    {
      name: `ParentStateVerifierFacet (${countFunctions(parentStateVerifierFacetAbi)} functions)`,
      value: parentStateVerifierFacetAbi,
    },
    {
      name: `UpwardMessageDispatcherFacet (${countFunctions(upwardMessageDispatcherFacetAbi)} functions)`,
      value: upwardMessageDispatcherFacetAbi,
    },
    {
      name: `ParentTokenMessageProcessorFacet (${countFunctions(parentTokenMessageProcessorFacetAbi)} functions)`,
      value: parentTokenMessageProcessorFacetAbi,
    },
    {
      name: `ParentTokenMessageTransmitterFacet (${countFunctions(parentTokenMessageTransmitterFacetAbi)} functions)`,
      value: parentTokenMessageTransmitterFacetAbi,
    },
    {
      name: `AccessControlFacet (${countFunctions(accessControlFacetAbi)} functions)`,
      value: accessControlFacetAbi,
    },
    {
      name: `TimelockFacet (${countFunctions(TimelockFacetAbi)} functions)`,
      value: TimelockFacetAbi,
    }
  ];
